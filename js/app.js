...
function openBand(band) {
  ...
  // ALWAYS show band cover when clicking sidebar band
  // NEVER interrupt playback
  mediaVideo.classList.add("hidden");   // hide video but keep it playing
    ...
    div.addEventListener("click", () => {
      ...
      if (isMobile()) {
        ...
        if (isVideoTrack) {
          ...
          // SAME VIDEO → just show injected video again
...
function getActivePlayer() {
  ...
  if (!audio.classList.contains("hidden")) return audio;
  if (!video.classList.contains("hidden")) return video;
  if (trackVideo && !trackVideo.paused && !trackVideo.ended && trackVideo.currentTime > 0) {
    return trackVideo;
  }
  // fallback
  return audio;
...
document.querySelectorAll(".tab-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    const targetId = btn.dataset.tab;
    if (targetId !== "tracks" && targetId !== "related") {
      hideTrackVideo();
    }
  });
});
